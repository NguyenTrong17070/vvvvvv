...
module.exports. run = function({ api, event, args, getText }) {
    ...
    const text = `🔰Trang (${page}/${Math.ceil(arrayInfo.length/numberOfOnePage)})🔰\n🔰Gõ: "${prefix}hép <tên lệnh>" để biết thêm chi tiết về lệnh đó🔰\n❗Hiện tại có ${arrayInfo.length} lệnh trên bot của Nguyễn Phát link Facebook: https://facebook.com/100075049159222❗\n❗Dùng ${prefix}hép <Số trang>❗\n_________________________________\n[🤔🧐Bạn có biết🤔🧐(Biết Cái Con Mẹ Máy)] : ${randomText[Math.floor(Math.random()*randomText.length)]}`;
