          ...
          switch (type) {
              ...
              case "sellmenu": {
                  ...
                  switch (body) {
                      ...
                      case "2": {
                          return api.sendMessage("bạn không có cá🎣", threadID, messageID);
                      } 
      ...
      module.exports.run = async function ({ event, api, args }) {
          ...
          switch ((args[0] || "").toLowerCase()) {
              ...
              case "-r": {
                  try {
                      if (existsSync(dirUser)) return api.sendMessage("[🎣Fishing🎣] Bạn đã từng đăng ký câu cá tại khu vực này! hãy gõ lệnh /fishing register đễ được đăng ký mục câu cá🎣", threadID, messageID);
                      var newData = {};
                      newData.fishingrod = {
                          "rodType": 0,
                          "enchantRod": {}
              ...
              case "shop": {
                  if (!existsSync(dirUser)) return api.sendMessage("[🎣Fishing Shop🛒] Bạn chưa đăng ký câu cá để có thể sử dụng chức năng mua đồ! hãy gõ lệnh /fishing register đễ được đăng ký mục câu cá🎣", threadID, messageID);
              ...
              default: {
                  try {
                      if (!existsSync(dirUser)) return api.sendMessage("[🎣Fishing🎣] Bạn chưa đăng ký câu cá tại khu vực này! hãy gõ lệnh /fishing register đễ được đăng ký mục câu cá🎣", threadID, messageID);
                      var dataUser = JSON.parse(readFileSync(dirUser, "utf-8"));
                      const dateNow = moment().tz("Asia/Ho_Chi_minh");
                      const format = new Intl.NumberFormat();
                      if (dataUser.fishingrod.rodType == 0) return api.sendMessage("[🎣Fishing🎣] Hiện tại bạn chưa có cần câu, hãy mua cần câu mới và thử lại! hãy gõ lệnh /fishing shop đễ mua được cần câu🎣", threadID, messageID);
                  ...
                  } catch (error) { console.log(error); return api.sendMessage("[🎣Fishing🎣]\n'bạn cần thời gian để cá xuất hiện", threadID, messageID) }
              }
          }
                         }